                        ...
                        try:
                            ...
                            try:
                                ...
                                if face_detection_error:
                                    logger.info("Face detection or model loading error: Adding audio to original video only")
                                else:
                                    logger.warning(f"Other error occurred: {error_msg}")
                                
                                # Combine original video and audio
                                logger.info("Alternative processing: Adding audio to original video only")
                                result = vhs_videocombine.combine_video(
                                    frame_rate=25,
                                    loop_count=0,
                                    filename_prefix=output_filename,
...
#             with gpu_manager.manager_lock:
#                 in_use = gpu_manager.gpu_in_use[i]
...
# def patch_unet_model():
#     """UNet 모델의 로딩 방식을 패치합니다."""
#     try:
#         unet_path = "/workspace/ComfyUI/custom_nodes/ComfyUI-LatentSyncWrapper/latentsync/models/unet.py"
...
#         # 백업 파일 생성 (아직 없는 경우)
#         backup_path = f"{unet_path}.backup"
#         if not os.path.exists(backup_path):
#             with open(backup_path, 'w') as f:
#                 f.write(content)
        
#         # 패치할 코드 찾기 (정확한 들여쓰기 유지를 위해 앞뒤 문맥을 더 포함)
#         original_code = "    unet = cls.from_config(model_config).to(device)"
#         if original_code in content:
#             # 패치 적용 (들여쓰기 유지)
#             modified_content = content.replace(
#                 original_code,
#                 """    # Try to load using to() method, fall back to to_empty() if needed
#         try:
#             unet = cls.from_config(model_config).to(device)
#         except NotImplementedError:
#             # meta tensor 처리를 위한 to_empty 메서드 사용
#             unet = cls.from_config(model_config).to_empty(device=device)"""
#             )
...
#             logger.info("Successfully patched UNet model loading")
#             return True
#         else:
#             logger.warning("Could not find target code in UNet model file")
#             return False
#     except Exception as e:
#         logger.error(f"Failed to patch UNet model: {str(e)}")
#         return False

# def patch_whisper_model():
#     """Whisper 모델의 로딩 방식을 패치합니다."""
#     try:
#         whisper_init_path = "/workspace/ComfyUI/custom_nodes/ComfyUI-LatentSyncWrapper/latentsync/whisper/whisper/__init__.py"
        
#         # 새로운 내용으로 파일 전체를 덮어씁니다
...
#         path to a model checkpoint containing the model dimensions and state dict.
#     device : Union[str, torch.device]
#         the PyTorch device to put the model into
#     download_root: str
#         path to download the model files; by default, it uses "~/.cache/whisper"
#     in_memory: bool
#         whether to preload the model weights into memory
...
#         # 파일 저장
#         with open(whisper_init_path, 'w') as f:
#             f.write(new_content)
        
#         logger.info("Successfully replaced Whisper model loading code")
#         return True
#     except Exception as e:
#         logger.error(f"Failed to patch Whisper model: {str(e)}")
#         return False

# 메인 함수에서 환경 설정 전에 패치 적용
if __name__ == "__main__":
    # Whisper 모델 패치 적용
    # logger.info("Patching Whisper model...")
    # if patch_whisper_model():
    #     logger.info("Whisper model patch applied successfully")
    # else:
    #     logger.warning("Failed to apply Whisper model patch")
    
    # UNet 모델 패치 적용
    # logger.info("Patching UNet model...")
    # if patch_unet_model():
    #     logger.info("UNet model patch applied successfully")
    # else:
    #     logger.warning("Failed to apply UNet model patch")

    # Face Alignment 라이브러리 패치 적용
